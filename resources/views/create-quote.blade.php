                ...
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" method="POST" action="/quotes/quote/create" enctype="multipart/form-data">
                        ...
                        @foreach (config('app.available_locales') as $locale)
                                <div>
                                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{__('forms.quote_name')}}({{ucwords($locale)}})</label>
                        ...
                        <div>
                            ...
                            <input placeholder='das-dsds-das-d-as-das-dasd-ad'  value="{{old('slug')}}" type="text" name="slug" id="slug" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            @error('slug')
                                <span class="text-xs text-red-500">{{$message}}</span>
                            @enderror
                        </div>
