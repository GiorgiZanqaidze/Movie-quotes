<x-panel>
    ...
    <form class="space-y-4 md:space-y-6" method="POST" action="/quotes/quote/{{$quote->id}}/edit" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <x-form.input name="name_en" placeholder="{{__('placeholders.quote_name')}}" label="{{__('forms.quote_name')}} ({{__('forms.en')}})">{{ old('name', $quote->name) }}</x-form-input>
        <x-form.input name="name_ka" placeholder="{{__('placeholders.quote_name')}}" label="{{__('forms.quote_name')}} ({{__('forms.ka')}})">{{ old('name', $quote->name) }}</x-form-input>
